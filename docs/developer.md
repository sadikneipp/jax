...
Bazel will always use its own hermetic Python interpreter and deps, only the 
`build/build.py` script itself will be processed by your system Python 
interpreter.
...
4) Check the output of the previous command. At the very end of it you will find
   a code stippet for `python_register_toolchains()` entry with your newly built
...
6) For unstable versions of Python, optionally (but highly recommended)
   ...
   sources with clang fails out of the box due to differences in LTO behaviro (
   ...
   expect any of your Python dependencies to be bilt from sources (i.e. binary
   distributions for the correspponding Python version already exist in the
...
# your dependencies are for the new versionf of Python.  
```
